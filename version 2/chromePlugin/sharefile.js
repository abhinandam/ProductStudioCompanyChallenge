        ...
        $.getJSON('http://ec2-35-163-235-152.us-west-2.compute.amazonaws.com/readmessage', { currUser: currUser }, function(data) {
            ...
            $.each(data.urls, function(index) {
                ...
                if (data.urls[index][6] == 1){
                    ...
                    $('.showresult').append("<div class='row read'><div class='col-xs-2'><img class='articleimage' src='"+data.urls[index][4]+"'></div><div class='col-xs-9'><p><a id='a"+data.urls[index][0]+"' target='_blank' href='"+data.urls[index][3]+"'>"+data.urls[index][5]+"</a></p><label>From: "+data.urls[index][7]+" "+data.urls[index][8]+"</label></div><div class='col-xs-1 rightstatus'></div></div><hr>");

                }
                
                if (data.urls[index][6] == 0){

                    $('.showresult').append("<div class='row'><div class='col-xs-2'><img class='articleimage' src='"+data.urls[index][4]+"'></div><div class='col-xs-9'><p><a id='a"+data.urls[index][0]+"' target='_blank' href='"+data.urls[index][3]+"'>"+data.urls[index][5]+"</a></p><label>From: "+data.urls[index][7]+" "+data.urls[index][8]+"</label></div><div class='col-xs-1 rightstatus'><span class='glyphicon glyphicon-certificate'></span></div></div><hr>");
